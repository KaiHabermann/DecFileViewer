...
function App() {
      ...
      selectedFilters.every(filter => {
        ...
        // If filter is a decay object { decay, topLevel }
          ...
          return decayStructures.some(modeDecays => {
            ...
            if (filter.topLevel) {
              // Only check the root of each mode
              return modeDecays.slice(0, 1).some(decay => decayContains(decay, filter.decay, true));
            }
            return modeDecays.some(decay => decayContains(decay, filter.decay));
          });
        }
        return false;
      });
  ...
  const addFilter = (filter) => {
    ...
    const entry = Array.isArray(filter) ? { decay: filter, topLevel: false } : filter;
